...
from outreach_bidi import transparent_bidi
...
.mobile-store-cta__link{display:flex;align-items:center;justify-content:center;width:100%;min-width:0;max-width:100%;min-height:48px;padding:6px 18px;border-radius:14px;background:linear-gradient(135deg,#4f55e8,#8057d9);box-shadow:0 7px 18px rgba(79,85,232,.28);color:#fff!important;text-decoration:none;font-size:clamp(.875rem,4vw,1rem);font-weight:850;letter-spacing:normal;line-height:1.4;white-space:normal;overflow:visible;overflow-wrap:anywhere;text-align:center;-webkit-tap-highlight-color:transparent}
.mobile-store-cta__label{margin-inline:auto;min-inline-size:0;max-inline-size:100%;unicode-bidi:isolate;direction:ltr}
...
@media(max-width:759px){body.mobile-store-cta-active{padding-bottom:calc(var(--mobile-store-cta-height,60px) + 22px + env(safe-area-inset-bottom))}}
...
`;
  ...
  const update = () => {
    ...
    if (height && height !== measuredHeight) {
      ...
      document.body.style.setProperty("--mobile-store-cta-height", `${height}px`);
    }
  ...
  if ("ResizeObserver" in window) new ResizeObserver(requestUpdate).observe(bar);
  update();
})();
"""
...
def _plain_label(source: str) -> str:
    without_tags = re.sub(r"<[^>]+>", " ", transparent_bidi(source))
