...
PAGES = HERE / "pages"
REVIEW_HUBS = PAGES / "review-hubs"
REVIEW_HUBS.mkdir(parents=True, exist_ok=True)
...
.stars{color:#f59e0b;font-size:1.2rem;margin:.5rem 0}
.dl{display:inline-block;background:#007aff;color:#fff;padding:.55rem 1.2rem;border-radius:8px;
  text-decoration:none;font-weight:600;font-size:.9rem;margin:1.2rem 0}
"""
...
def build_review_hub(app_cfg, lang_dirs):
    ...
    link_items = "\n".join(
        f'<a class="lang-link" href="{url}" hreflang="{lang}">{lname}</a>'
        for lang, lname, url in links
    )
    ld = json.dumps([
        {
        ...
        },
        {
            "@context": "https://schema.org",
            "@type": "ItemList",
            "name": f"{name} Review — {len(links)} Languages",
            "numberOfItems": len(links),
            "itemListElement": [
                {"@type": "ListItem", "position": i+1, "url": url, "name": lname}
                for i, (lang, lname, url) in enumerate(links)
            ]
        }
    ], ensure_ascii=False)

    hub_slug = f"{key}-review-all-languages"
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>{name} Review 2026 — {len(links)} Languages</title>
<meta name="description" content="Read honest {name} reviews in {len(links)} languages. Pay-once iOS app — no subscription.">
<meta name="robots" content="index,follow">
<link rel="canonical" href="{GEO_SITE}/review-hubs/{hub_slug}.html">
<style>{CSS}</style>
...
<p style="margin-top:2rem;font-size:.85rem;color:#888">
  ...
  Reviews contributed by the developer. See each language version for detailed local pricing.
</p>
</body>
</html>"""
    return html, len(links)

def build_index(apps_built):
    items = "\n".join(
        f'<a class="lang-link" style="font-size:.95rem" href="{GEO_SITE}/review-hubs/{key}-review-all-languages.html">{emoji} {name} Review ({count} languages)</a>'
        for key, name, emoji, count in apps_built
    )
...
def main():
    ...
    for app_cfg in REVIEW_APPS:
        html, count = build_review_hub(app_cfg, lang_dirs)
        if html and count > 0:
            key = app_cfg["key"]
            name = APPS.get(key, {}).get("name", key)
            hub_slug = f"{key}-review-all-languages"
            (REVIEW_HUBS / f"{hub_slug}.html").write_text(html, encoding="utf-8")
            apps_built.append((key, name, app_cfg["emoji"], count))
            sm_urls.append(f"{GEO_SITE}/review-hubs/{hub_slug}.html")

    if apps_built:
        (REVIEW_HUBS / "index.html").write_text(build_index(apps_built), encoding="utf-8")
        sm_urls.insert(0, f"{GEO_SITE}/review-hubs/")

    sm = '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
    sm += "\n".join(f"<url><loc>{u}</loc></url>" for u in sm_urls) + "\n</urlset>\n"
    (PAGES / "sitemap_review_hubs.xml").write_text(sm, encoding="utf-8")

    print(json.dumps({"review_hubs": len(apps_built), "apps": [f"{k}:{c}" for k,_,_,c in apps_built]}))
