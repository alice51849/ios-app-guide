        ...
        def visit(value):
            if isinstance(value, dict):
                ...
                if isinstance(types, list) and APP_TYPES.intersection(types):
                    ...
                    if isinstance(identity, dict) and identity.get("propertyID") == "Apple App Store ID":
...
def unavailable_json(value, locale: str):
    if not market.is_unavailable(locale):
        return value
    if isinstance(value, list):
        return [unavailable_json(item, locale) for item in value]
    ...
    for key, child in value.items():
        ...
        if identity_ids and "apple.com" in child:
            ...
            result[key] = unavailable_json(child, locale)
    if application:
        if app_ids:
            result["identifier"] = {
                "@type": "PropertyValue", "propertyID": "Apple App Store ID", "value": app_ids[0],
            }
        result.update(market.record_fields(locale))
    return result
...
def enforce_html(source: str, locale: str | None, *, app_id=None, name=None) -> str:
    ...
    def script(match):
        payload = json.loads(match[2])
        payload = unavailable_json(payload, locale)
    ...
    def store_anchor(match):
        ...
        if needs_note:
            needs_note = False
            return match[1] + market.note_html(locale, name)
        return match[1]
    source = re.sub(
    ...
    source = STORE.sub("", source)
    if "apps.apple.com" in source or "itunes.apple.com" in source:
    ...
    if existing:
        ...
        replacement = market.note_html(locale, name)
        if name is None and len(NOTE.findall(source)) == 1 and old_text.startswith("Apple App Store এখনো বাংলাদেশে"):
            replacement = replacement.replace(html.escape(market.note(locale)), html.escape(old_text))
