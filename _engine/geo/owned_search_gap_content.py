...
def strings(page):
    return [page["heading"], page["disclosure"], *page["paragraphs"],
            ...
            *[item["after"] for item in page.get("body_corrections", [])]]
...
def render(source: bytes, page: dict, queries: dict, corpus: dict):
    ...
    correction_count = 0
    body_match = re.search(r"<body\b[^>]*>([\s\S]*?)</body>", output, re.I)
    if not body_match:
        raise ValueError("A single body boundary is required")
    body = body_match.group(1)
    for correction in page.get("body_corrections", []):
        if (set(correction) != {"before", "after"} or not correction["before"]
                or body.count(correction["before"]) != 1):
            raise ValueError("Reviewed body-only claim correction is absent or ambiguous")
        body = body.replace(correction["before"], correction["after"], 1)
        correction_count += 1
    output = output[:body_match.start(1)] + body + output[body_match.end(1):]
    ...
    return output.encode(), {**metrics, "path": page["path"], "before_sha256": sha(source),
                              ...
                              "reviewed_body_claim_corrections": correction_count,
