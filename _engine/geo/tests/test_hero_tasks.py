...
class HeroTaskTests(unittest.TestCase):
    ...
    def test_buttondown_is_localized_idempotent_and_bound_before_output_hashes(self):
        ...
        for row in manifest["records"]:
            ...
            expected = hero.email_capture._block(config, row["locale"])
            self.assertEqual([expected] if expected else [], blocks)
            ...
            preview = hero.email_capture._block(config, row["locale"], preview=True)
            label = re.search(r"<label\b[^>]*>(.*?)</label>", preview).group(1)
            self.assertNotIn("@", label)
            self.assertIn('name="email"', preview)
            self.assertIn('name="embed" value="1"', preview)
