...
class OwnedContentTests(unittest.TestCase):
    ...
    def fixture(self, page):
        ...
        legacy = "".join("<p>" + html.escape(item["before"]) + "</p>" for item in page.get("body_corrections", []))
        text = (f'<html lang="{page["locale"]}"><head><title>Existing title</title>'
                ...
                '<body><main><section><h2>Existing useful answer</h2><p>Keep this existing paragraph intact.</p></section>' + legacy +
    ...
    def test_shipping_diagnostic_claim_is_corrected_only_in_body(self):
        page, raw = self.fixture(self.plan["pages"][0])
        updated, metrics = subject.render(raw, page, self.queries, self.corpus)
        self.assertEqual(1, metrics["reviewed_body_claim_corrections"])
        self.assertNotIn(page["body_corrections"][0]["before"].encode(), updated)
        self.assertIn(page["body_corrections"][0]["after"].encode(), updated)
        self.assertTrue(metrics["head_byte_identical"])

    def test_body_claim_correction_does_not_guess_missing_or_duplicate_text(self):
        page, raw = self.fixture(self.plan["pages"][0])
        before = page["body_corrections"][0]["before"].encode()
        changed = raw.replace(before, b"already edited")
        page["base_sha256"] = subject.sha(changed)
        with self.assertRaisesRegex(ValueError, "absent or ambiguous"):
            subject.render(changed, page, self.queries, self.corpus)
