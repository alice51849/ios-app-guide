---
title: "HoursTag Lite — 看見每樣東西的真實代價 | iOS App"
lang: "zh-Hant"
canonical: "https://alice51849.github.io/ios-app-guide/apps/hourstaglite/decision/l/zh-Hant/index.html"
modified: "2026-07-28"
app_store_id: "6794039979"
verified_live: true
purchase_model: "free_with_lifetime_unlock"
publisher: "Lumi Studio"
---

# HoursTag Lite — 看見每樣東西的真實代價 | iOS App

HoursTag Lite - 看見每樣東西的真實代價 每一筆花費，都耗掉你生命中的工時。HoursTag Lite 把價格換算成工作時間，讓你花得清楚。 認識你的價值稜鏡 - 人生時間成本: 每工作一小時的實領價值 - 類型: 必要 - 想要 - 衝動 - 你的價值模式 - 完整價值洞察 - 這個目標要花多少時間？: 看見一個目標背後的工時，並追蹤進度。 - 備份與還原: 包含收入、花費、目標與偏好設定。 - Widget - Apple Watch 所有資料只留在你的裝置上。無需帳號，不做追蹤。 一次付費 · 永久使用 · 絕無訂閱 無限選擇與編輯 無限目標與進度 分享、洞察與備份

- 理財與旅行
- 免費開始 · 永久解鎖
- 免帳號
- 無追蹤
- 隱私優先／裝置端
- 主畫面小工具
- Apple Watch
- App Store · 免費

[在 App Store 取得 HoursTag Lite](https://apps.apple.com/tw/app/id6794039979)

[閱讀完整指南](https://alice51849.github.io/ios-app-guide/zh-Hant/hourstaglite.html)

> 這是由 Lumi Studio 發布的第一方資料；Lumi Studio 是每款列出 App 的開發者。
